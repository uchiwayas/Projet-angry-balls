...
public class Collisions {
	...
	/**
	 * gestion de la collision avec traversement de la paroi (et réapparition
	 * sur le coté opposé) de la bille définie par (position) avec un contour
	 * rectangulaire de l'écran. Ce rectangle est défini par
	 * (abscisseCoinHautGauche, OrdonnéeCoinHautGauche,largeur,hauteur) si il
	 * n'y a pas collision, le vecteur position n'est pas modifié si il y a
	 * collision, le vecteur position est modifié : la bille "ressort" sur le
	 * bord opposé au bord qu'elle pénètre.
	 * 
	 * @param position
	 *            : vecteur position de la bille
	 * 
	 */
	public static void collisionBilleContourPasseMuraille(Vecteur position, double abscisseCoinHautGauche,
	...
	/**
	 * gestion de la collision avec arrêt suivant la direction horizontale de la
	 * bille définie par (position,rayon,vitesse) avec un contour rectangulaire
	 * de l'écran.
	 * 
	 * Ce rectangle est défini partiellement par
	 * (abscisseCoinHautGauche,largeur)
	 * 
	 * si il n'y a pas de collision avec un bord vertical, position et vitesse
	 * sont inchangés par la méthode
	 * 
	 * si il y a collision avec un bord vertical alors vitesse est modifié et
	 * position reste intact
	 * 
	 * Dès qu'il y a collision avec un bord vertical, la composante horizontale
	 * du vecteur vitesse est annulée, de sorte que la bille continue à glisser
	 * le long de la bande qui l'a arrêtée
	 * 
	 */
	public static void collisionBilleContourAvecArretHorizontal(Vecteur position, double rayon, Vecteur vitesse,
		...
		// vitesse.x = Collisions.arretSurBord(vitesse.x, position.x, rayon,
		// abscisseCoinHautGauche, largeur);
	...
	/**
	 * gestion de la collision avec arrêt suivant la direction verticale de la
	 * bille définie par (position,rayon,vitesse) avec un contour rectangulaire
	 * de l'écran.
	 * 
	 * Ce rectangle est défini partiellement par
	 ...
	 * 
	 * si il n'y a pas de collision avec un bord horizontal, position et vitesse
	 * sont inchangés par la méthode
	 * 
	 * si il y a collision avec un bord horizontal alors vitesse est modifié et
	 * position reste intact
	 * 
	 * Dès qu'il y a collision avec un bord horizontal, la composante verticale
	 * du vecteur vitesse est annulée, de sorte que la bille continue à glisser
	 * le long de la bande qui l'a arrêtée
	 * 
	 */
	public static void collisionBilleContourAvecArretVertical(Vecteur position, double rayon, Vecteur vitesse,
		...
		// vitesse.y = Collisions.arretSurBord(vitesse.y, position.y, rayon,
		// ordonnéeCoinHautGauche, hauteur);
	...
	/*
	 * utile à collisionBilleContourAvecArretHorizontal et à
	 * collisionBilleContourAvecArretVertical
	 */
	private static double[] arretSurBord(double v, double x, double rayon, double xMin, double largeur) {
		double[] t = new double[2]; // nouvelle composante_vitesse dans t[0] et
									// nouvelle composante_position dans t[1]
	...
	/**
	 * gestion de la collision avec rebond de la bille définie par
	 * (position,rayon,vitesse) avec le segment orienté [P0P1]
	 * 
	 ...
	 * 
	 * 
	 *         c-à-d que en entrée on considère que (position,vitesse) sont le
	 *         vecteur position et le vecteur vitesse de la bille immédiatement
	 *         avant le choc et en sortie (position,vitesse) sont le vecteur
	 *         position et le vecteur vitesse de la bille immédiatement après le
	 *         choc
	 * 
	 *         le vecteur vitesse est modifié par la collision (comme une boule
	 *         de billard l'est par une bande) La méthode laisse le vecteur
	 *         position intact
	 * @param position
	 *            : vecteur position de la bille immédiatement avant la
	 *            collision avec le segment [P0P1]
	 * @param rayon
	 *            : rayon de la bille
	 * @param vitesse
	 *            : vecteur vitesse de la bille immédiatement avant la collision
	 *            avec le segment [P0P1]
	 * @param P0
	 *            : le début du segment
	 * @param P1
	 *            : la fin du segment
	...
	public static boolean collisionBilleSegmentAvecRebond(final Vecteur position, double rayon, Vecteur vitesse,
		...
		if (distanceSignée >= rayon)
			return false; // il n'y a pas collision entre la bille
							// (position,rayon) et le segment [P0P1]

		double vN = vitesse.produitScalaire(N);

		if (vN >= 0)
			return false; // la bille est à l'extérieur et revient vers le cadre
							// ou est à l'intérieur et s'éloigne du bord
	...
	/**
	 * gestion de la collision avec rebond de la bille définie par
	 * (position,rayon,vitesse) avec un contour rectangulaire de l'écran.
	 * 
	 * Ce rectangle est défini par (abscisseCoinHautGauche,
	 ...
	 *            : abscisse minimale du contour rectangulaire
	 * @param ordonnéeCoinHautGauche
	 *            : ordonnée minimale du contour rectangulaire
	 * @param largeur
	 *            : largeur du contour rectangulaire
	 * @param hauteur
	 *            : hauteur du contour rectangulaire
	 * 
	 * 
	 */
	public static boolean collisionObjetContourAvecRebond(final Vecteur position, double rayon, Vecteur vitesse,
		...
		coins[0] = min; // le coin haut gauche du rectangle défini par le
						// composant
	...
	/**
	 * Effectue les calculs correspondants à la collision entre la bille
	 * b1(G1,rayon1,v1,m1) et la bille b2(G2,rayon2,v2,m2)
	 * 
	 * La méthode laisse toujours G1 et G2 intacts.
	 *
	 * @param G1
	 *            : centre de la bille b1
	 * @param rayon1
	 *            : rayon de la bille b1
	 * @param v1
	 *            : vecteur vitesse de la bille b1
	 * @param m1
	 *            : masse de la bille b1
	 * 
	 * @param G2
	 *            : centre de la bille b2
	 * @param rayon2
	 *            : rayon de la bille b2
	 * @param v2
	 *            : vecteur vitesse de la bille b2
	 * @param m2
	 *            : masse de la bille b2
	 * 
	 * @return true si il y a collision et false si il n'y a pas collision
	 * 
	 *         si retourne true, modifie les vecteurs vitesse v1 et v2
	 */
	public static boolean CollisionObjetObjet(final Vecteur G1, double rayon1, Vecteur v1, double m1, final Vecteur G2,
			double rayon2, Vecteur v2, double m2, double width)
	// modifie b1 et b2
	...
	// résultats : b1 et b2 après le choc
	{
		...
		if (nG1G2_2 >= r2)
			return false; // il n'y a pas de collision entre les 2 billes car
							// elles sont trop éloignées l'une de l'autre

		double nG1G2 = Math.sqrt(nG1G2_2);

		Vecteur N = G1G2.produit(1 / nG1G2); // vecteur unitaire sur le segment
												// [G1 G2]
		...
		Son.sonCollisionObjetObjet(a, pointDeCollision, width);
		// FIN SON

		if (a <= 0)
			return false; // les billes séloignent l'une de l'autre, la
							// collision a donc déjà été traitée
